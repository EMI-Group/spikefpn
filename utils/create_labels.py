...
def compute_iou(anchor_boxes, gt_box):
    ...
    # Then compute IoU between anchor_box and gt_box
    S_gt = w_gt * h_gt
    S_ab = w_ab * h_ab
...
def label_assignment_with_anchorbox(anchor_size, target_boxes, num_anchors, strides, multi_anchor=False):
    ...
    if multi_anchor:
        # Consider those anchor boxes whose IoU is more than 0.5,
        iou_mask = (iou > 0.5)
        if iou_mask.sum() == 0:
    ...
    for bi in range(batch_size):
        ...
        for box_cls in label:
            ...
            if anchor_size is not None:
                ...
                # No anchor box
                label_assignment_results = label_assignment_without_anchorbox(target_boxes=target_boxes, strides=strides)
            ...
            for result in label_assignment_results:
                ...
                if center_sample:
                    # Consider four grid points near the center point
                    for j in range(grid_y, grid_y+2):
                        for i in range(grid_x, grid_x+2):
                            if (j >= 0 and j < gt_tensor[scale_ind].shape[1]) and (i >= 0 and i < gt_tensor[scale_ind].shape[2]):
                                ...
                                gt_tensor[scale_ind][bi, j, i, anchor_ind, 2:6] = np.array([x1, y1, x2, y2]) / img_size
                                gt_tensor[scale_ind][bi, j, i, anchor_ind, 6] = box_scale
                else:
                    # Only consider top-left grid point near the center point
                    if (grid_y >= 0 and grid_y < gt_tensor[scale_ind].shape[1]) and (grid_x >= 0 and grid_x < gt_tensor[scale_ind].shape[2]):
                        ...
                        gt_tensor[scale_ind][bi, grid_y, grid_x, anchor_ind, 2:6] = np.array([x1, y1, x2, y2]) / img_size
